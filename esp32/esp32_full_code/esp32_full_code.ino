...
volatile unsigned long long sample_time = 0;
...
void deserializeJSON() {
  ...
  JsonObject object = JSON_DOC_RECEIVE.as<JsonObject>();
  ...
  Serial.print("Memory usage of RECEIVING DATA (bytes): ");
  Serial.println(object.memoryUsage());
...
void sendJSON() {
  ...
  JsonObject object = JSON_DOC_RECEIVE.as<JsonObject>();
  Serial.print("Memory usage of SENDING DATA (bytes): ");
...
void loop() {
  ...
  sample_time = micros() - timerPivot;
  if (sample_time >= PERIOD) {
    deserializeJSON();
    packageData();
    sendJSON();
    //    Serial.print("Left encoder tick per second: ");
    //    Serial.println((int32_t)encoder_1.getCount());
    //    Serial.print("Right encoder tick per second: ");
